...
def latest(iso3: str):
    """Get most recent year row for a country."""
...
def root():
    return {
        "name": "PRISM API",
        "version": "1.0.0",
        "endpoints": ["/score/{iso3}", "/rankings", "/trend/{iso3}", "/countries"]
    }

@app.get("/score/{iso3}")
def get_score(iso3: str):
    """Get risk scores for a country by ISO3 code."""
    row = latest(iso3)
    if row is None:
    ...
    return {
        ...
        "dimensions": {
            ...
            for dim in RISK_DIMS if dim in row and pd.notna(row[dim])
        }
    }

@app.get("/rankings")
def get_rankings(year: int = None, limit: int = 200):
    """Get all countries ranked by PRISM score."""
    data = df.copy()
    if year:
...
def get_trend(iso3: str):
    """Get year-by-year risk trend for a country."""
